...
# Final Optimized Version: Safety First - Tích hợp Prompt thông minh và Cơ chế sinh an toàn.
...
debug_corrections_folder = "./debug_vihallu_public_test_correction"
os.makedirs(debug_corrections_folder, exist_ok=True)

# ---------------- CONFIG ----------------
MODEL = "VietnamAIHub/Vietnamese_llama2_7B_8K_SFT_General_domain"
INPUT_CSV = "./vihallu-public-test.csv"
OUTPUT_CSV = os.path.join(debug_corrections_folder, "./fixed-vihallu-public-test_final_v7_temp.csv")
COLUMNS_TO_FIX = ['context', 'prompt', 'response']

# Bộ FEW_SHOT đầy đủ và thông minh nhất
FEW_SHOT = [
...
# Các ngưỡng lọc đã được cân bằng
...
# debug outputs
correction_cache = {}
correction_villua_folder = "./vihallu_public_correction"
os.makedirs(correction_villua_folder, exist_ok=True)

DEBUG_LOG_CSV = os.path.join(correction_villua_folder, "debug_corrections_final_v7_temp.csv")
SAMPLE_DEBUG_N = 20
...
# ... (Phần setup và các hàm tiện ích giữ nguyên cấu trúc của bạn)
# Ctrl+C handler
df_global = None
...
def generate_from_model(prompt, original_text_for_token_limit):
    ...
    # *** UPDATE: Khôi phục min_new_tokens để đảm bảo an toàn tuyệt đối ***
    ...
    gen_cfg = GenerationConfig(
        ...
        early_stopping=False, # Đặt là False khi dùng min_new_tokens
...
def correct_vietnamese_spelling(text, row_id, col_name, debug_print=False):
    ...
    if corrected_text and corrected_text != original_text:
        # Giữ nguyên logic lọc đã được cân bằng
    ...
    if debug_print:
        ...
        print(f"ORIGINAL: \n{original_text}")
        print(f"CORRECTED (model returned): \n{corrected_text}")
